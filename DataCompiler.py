...
def convertToTable(totalCount, typeCounts, userCounts):

#    typeCounts = sorted(typeCounts[1:], key=operator.itemgetter(1), reverse = True)
    
    numericData = [["User", "Overall"], ["TOTAL", totalCount]]
    percentData = [["User", "Overall"], ["TOTAL", totalCount]]
    typeData = sorted(typeCounts.items(), key=operator.itemgetter(1), reverse = True)
    for blockType, blockCounts in typeData:
        numericData[0].append(blockType)
        percentData[0].append(blockType)
        numericData[1].append(blockCounts[0])
        percentData[1].append(blockCounts[1])

    userCounts["[ANONYMOUS]"] = userCounts[""]
    userCounts.pop("")
    userData = sorted(userCounts.items(), key=operator.itemgetter(1), reverse = True)

    for user, userCounts in userData:
        numericRow = [user, userCounts[0]]
        percentRow = [user, userCounts[1]]
        userTypeCounts = userCounts[2]

        for typeEntry in typeData:
            blockType = typeEntry[0]
            if blockType in userTypeCounts.keys():
                numericRow.append(userTypeCounts[blockType][0])
                percentRow.append(userTypeCounts[blockType][1])
            else:
                numericRow.append("-")
                percentRow.append("-")
                
        numericData.append(numericRow)
        percentData.append(percentRow)

    return numericData, percentData
...
def main():
    ...
    numericData, percentData = convertToTable(totalCount, typeCounts, userCounts)
    ...
    EtSheet.saveCsv("numeric.csv", numericData)
    EtSheet.saveCsv("percent.csv", percentData)
    print "Spreadsheets saved."
#        print row[0] + ":\t" + str(row[1]) + " (" + str(row[2] * 100) + "%)"
